...
function balanceFontSize(amount: number): string {
  if (amount >= 100_000_000) return "text-2xl font-bold text-[#3A6B50]";
  if (amount >= 1_000_000)   return "text-3xl font-bold text-[#3A6B50]";
  return "text-4xl font-bold text-[#3A6B50] md:text-5xl";
}
...
const CustomDotLabel = (props: any) => {
  ...
  return (
    <g>
      <circle cx={x} cy={y} r={4} fill="#3A6B50" stroke="#FAF8F4" strokeWidth={2} />
      <text x={x} y={y - 12} textAnchor="middle" fill="#3A6B50" fontSize={11} fontWeight="600">
        {formatted}
      </text>
    </g>
  );
  ...
  return (
    <div className="min-h-screen bg-[#EDE8DF] text-[#3D2010]">
      <div className="container mx-auto max-w-2xl px-4 pt-14 pb-0 sm:px-6 lg:px-8">
        ...
        <p className="mt-2 text-center text-sm text-[#9C7A62] sm:text-left">
          See what your money can become.
        </p>
        {accountType && (
          <div className="mt-2 text-center sm:text-left">
            <span className="inline-block px-3 py-1 bg-[#DFF0E8] border border-[#A8C8B4] rounded-full text-xs text-[#3A6B50] font-medium">
        ...
        {/* Calculator card */}
        <div className="relative mt-10 overflow-hidden rounded-xl border border-[#DDD0BC] bg-[#FAF8F4] p-6 shadow-md md:p-8">
          {/* Faint green top accent line */}
          <div className="absolute inset-x-0 top-0 h-[2px] bg-[#3A6B50]/40" />
          ...
          <form className="space-y-5">
            ...
            {accountType && ACCOUNT_INFO[accountType] && (
              <div className="rounded-lg border border-[#A8C8B4] bg-[#EAF2EC] p-4">
                <h3 className="mb-2 text-lg font-semibold text-[#3A6B50]">
                  {accountType === "General" ? "Non-Registered Account" : accountType}
                </h3>
                <p className="mb-2 text-sm text-[#6B4E38]">
            ...
            {!accountType && (
              <p className="text-sm text-[#9C7A62] mt-2">Select an account type above to get started.</p>
            )}

            {/* Account Contribution Warning */}
            {accountWarning && (
              <div className="mt-3 p-4 bg-[#FBF3E0] border border-[#D4A840] rounded-lg">
                <p className="text-sm text-[#9C6B1A]">⚠️ {accountWarning}</p>
              </div>
            )}

            {/* Account contribution confirmation */}
            {accountConfirmation && (
              <div className="mt-3 p-4 bg-[#EAF2EC] border border-[#A8C8B4] rounded-lg">
                <p className="text-sm text-[#3A6B50]">{accountConfirmation}</p>
              </div>
            )}
            ...
            <div className={accountType ? "space-y-5" : "space-y-5 opacity-30 pointer-events-none select-none"}>
              {!accountType && (
                <div className="text-center py-4">
                  <p className="text-sm text-[#B89A82]">Choose an account type above to unlock the calculator.</p>
                </div>
              )}
            ...
            <div>
              ...
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#9C7A62]">$</span>
                <input
                  type="number"
            ...
            {accountType === "TFSA" && (
              <>
                <div>
                  ...
                  <p className="mt-1.5 text-xs text-[#9C7A62]">
              ...
              <>
                ...
                {(parseFloat(respAnnualContribution) || 0) > 0 && (
                  <p className="text-sm text-[#6B4E38]">
                    ...
                    <span className="font-semibold text-[#3A6B50]">
            ...
            <div>
              ...
              <div className="relative">
                ...
                />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[#9C7A62]">%</span>
              </div>
            </div>
          ...
          {/* ── Results ── */}
          {results && (
            <div className="mt-8 border-t border-[#DDD0BC] pt-8">
              <div className="space-y-6">
                {/* Projected Balance */}
                <div className="rounded-lg bg-[#EAF2EC] border border-[#A8C8B4] p-4">
                  <p className="mb-1 text-sm font-medium text-[#9C7A62] uppercase tracking-widest">
                  ...
                  <p className="mb-2 text-xs uppercase tracking-widest text-[#9C7A62]">
                    Projected Balance
                  </p>
            ...
            <div className="mt-8 border-t border-[#DDD0BC] pt-8">
              ...
              <div className="h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart
                    data={chartData}
                    margin={{ top: 36, right: 60, left: 0, bottom: 0 }}
                  >
                    <defs>
                      <linearGradient id="colorBalance" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%"  stopColor="#3A6B50" stopOpacity={0.25} />
                        <stop offset="95%" stopColor="#3A6B50" stopOpacity={0} />
                      </linearGradient>
                      <linearGradient id="colorContributed" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%"  stopColor="#C8B89A" stopOpacity={0.5} />
                        <stop offset="95%" stopColor="#C8B89A" stopOpacity={0} />
                      </linearGradient>
                    </defs>
                    <CartesianGrid strokeDasharray="3 3" stroke="#DDD0BC" vertical={false} />
                    <XAxis
                      dataKey="year"
                      stroke="#C8B89A"
                      tick={{ fill: "#9C7A62", fontSize: 12 }}
                      tickLine={{ stroke: "#DDD0BC" }}
                    />
                    <YAxis
                      stroke="#C8B89A"
                      tick={{ fill: "#9C7A62", fontSize: 12 }}
          ...
          {comparisonScenarios && comparisonScenarios.length > 0 && (
            <div className="mt-8 border-t border-[#DDD0BC] pt-8">
              ...
              <p className="mb-6 text-xs text-[#9C7A62]">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {comparisonScenarios.map((scenario) => (
                  ...
                  >
                    ...
                    <hr className="my-4 border-[#DDD0BC]" />

                      {!scenario.highlight && scenario.missedAmount > 0 && (
                      <>
                          <p className="text-xs text-[#9C7A62]">Lost Returns</p>
                        <p className={`mt-1 ${cardNumberFontSize(scenario.missedAmount)} text-[#B84535]`}>
              ...
              {comparisonScenarios.length >= 3 && comparisonScenarios[2].missedAmount > 0 && (
                <p className="text-sm text-[#9C7A62] mt-4 text-center">
        ...
        {/* Footer */}
        <p className="pb-4 pt-8 text-center text-xs text-[#B89A82]">
